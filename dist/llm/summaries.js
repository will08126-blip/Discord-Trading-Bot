...
exports.explainTrade = explainTrade;
...
function getClient() {
    if (!client) {
        if (!config_1.config.anthropic.apiKey) {
            throw new Error('ANTHROPIC_API_KEY is not set — LLM summaries disabled');
        }
        client = new sdk_1.default({ apiKey: config_1.config.anthropic.apiKey });
    }
    return client;
}
...
async function generateDailySummary() {
    try {
        const context = (0, tracker_1.buildDailySummaryContext)();
        const stats = (0, tracker_1.computeStats)((0, tracker_1.loadTrades)().filter((t) => new Date(t.closedAt).toISOString().slice(0, 10) === new Date().toISOString().slice(0, 10)));
        if (stats.totalTrades === 0) {
            return '📊 **Daily Summary** — No trades completed today.';
        }
        const response = await callClaude(SYSTEM_PROMPT, `Here is today's trading data:\n\n${context}\n\nPlease provide a brief daily performance summary.`);
        return `📊 **Daily Summary**\n\n${response}`;
    }
    catch (err) {
        logger_1.logger.error('Failed to generate daily summary:', err);
...
async function generateWeeklySummary() {
    try {
        const context = (0, tracker_1.buildWeeklySummaryContext)();
        const weekAgo = Date.now() - 7 * 24 * 60 * 60 * 1000;
        const stats = (0, tracker_1.computeStats)((0, tracker_1.loadTrades)().filter((t) => t.closedAt >= weekAgo));
        if (stats.totalTrades === 0) {
            return '📈 **Weekly Summary** — No trades completed this week.';
        }
        const response = await callClaude(SYSTEM_PROMPT, `Here is this week's trading data:\n\n${context}\n\nPlease provide a weekly performance summary with key insights.`);
        return `📈 **Weekly Summary**\n\n${response}`;
    }
    catch (err) {
        logger_1.logger.error('Failed to generate weekly summary:', err);
...
async function explainTrade(trade) {
    try {
        ...
        const response = await callClaude(SYSTEM_PROMPT, `Please briefly explain this trade outcome and what the data suggests about why it performed this way:\n\n${context}`);
        return `🔍 **Trade Analysis**\n\n${response}`;
    }
    catch (err) {
        logger_1.logger.error('Failed to explain trade:', err);
...
function fallbackDailySummary() {
    ...
    const todayTrades = (0, tracker_1.loadTrades)().filter((t) => new Date(t.closedAt).toISOString().slice(0, 10) === today);
    const stats = (0, tracker_1.computeStats)(todayTrades);
...
function fallbackWeeklySummary() {
    const weekAgo = Date.now() - 7 * 24 * 60 * 60 * 1000;
    const weekTrades = (0, tracker_1.loadTrades)().filter((t) => t.closedAt >= weekAgo);
    const stats = (0, tracker_1.computeStats)(weekTrades);
    const pnlStr = stats.totalPnlDollar >= 0 ? `+$${stats.totalPnlDollar.toFixed(2)}` : `-$${Math.abs(stats.totalPnlDollar).toFixed(2)}`;
    return [
        `📈 **Weekly Summary**`,
        `Trades: ${stats.totalTrades} | W: ${stats.wins} L: ${stats.losses} | WR: ${(stats.winRate * 100).toFixed(0)}%`,
        ...
        `\nStrategy breakdown:`,
        ...Object.entries(stats.byStrategy).map(([n, s]) => `  ${n}: ${s.totalTrades} trades, ${(s.winRate * 100).toFixed(0)}% WR, avg score ${s.avgScore.toFixed(1)}`),
    ].join('\n');
}
