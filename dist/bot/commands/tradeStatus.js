 ...
 */
function gradePosition(position, currentPrice, currentRsi) {
    ...
    const typicalHours = position.signal.tradeType === 'SCALP' ? 1
    ...
    if (progressPct >= 100) {
        ...
        if (rsiOverbought)
            commentary += ' RSI is extended; consider tightening the stop to protect gains.';
    }
        ...
        if (rsiOverbought)
            commentary += ' RSI is overextended; watch for a short-term pullback.';
    }
        ...
        if (rsiMomentumFailed)
            commentary += ' Momentum indicators are turning against the trade.';
    }
    else {
        ...
        if (rsiMomentumFailed)
            commentary += ' Momentum has failed — consider cutting the loss early to preserve capital.';
    }
...
// ─── Embed builder ───────────────────────────────────────────────────────────
async function buildStatusEmbed(position) {
    ...
    return new discord_js_1.EmbedBuilder()
        ...
        name: '💰 Prices',
        value: [
            `Entry:   **${(0, riskCalculator_1.formatPrice)(position.entryPrice, assetBase)}**`,
            `Current: **${(0, riskCalculator_1.formatPrice)(currentPrice, assetBase)}**`,
            `SL:      ${(0, riskCalculator_1.formatPrice)(position.currentStopLoss, assetBase)}`,
            `TP:      ${(0, riskCalculator_1.formatPrice)(position.currentTakeProfit, assetBase)}`,
        ].join('\n'),
        inline: true,
    }, {
        name: '📊 Performance',
        value: [
            `P&L:    **${(0, riskCalculator_1.formatPct)(grade.pnlPct)}**`,
            `R-mult: **${rMultipleStr}**`,
...
async function execute(interaction) {
    ...
    let positions = (0, signalManager_1.getAllActivePositions)();
